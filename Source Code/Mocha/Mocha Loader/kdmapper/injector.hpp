...
bool inject(LPCWSTR dll_path, SOCKET connection, PVOID mod_image = nullptr)
{
	PVOID dll_image = mod_image;
	...
	if (dll_image == nullptr) {
		dll_image = get_dll_by_file(dll_path);
		if (!dll_image)
		{
			SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
			std::wcout << skCrypt("[!] Invalid Mod \"") << filename << skCrypt("\"!\n");
	...
	{
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
		printf(skCrypt("[!] Error Code 0x10!\n"));
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
		return false;
	}

	DWORD thread_id;
	DWORD process_id = get_process_id_and_thread_id_by_window_class(skCrypt("UnityWndClass"), &thread_id);
	...
	if (process_id != 0 && thread_id != 0)
	{
		...
		*/
		...
		if (!relocate_image(allocate_base, dll_image, dll_nt_head))
		{
			...
			SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
			printf(skCrypt("[!] Error Code: 0x11\n"));
			SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
		}
		...
		if (!resolve_import(process_id, thread_id, dll_image, dll_nt_head, connection))
		{
			driver::free_memory(connection, process_id, (uintptr_t)allocate_base);
			SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_INTENSITY);
			printf(skCrypt("[!] Error Code: 0x12\n"));
			SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
			return false;
		}
		...
		std::wcout << skCrypt("[+] Successfully Loaded ") << filename << skCrypt("!\n");
		SetConsoleTextAttribute(hConsole, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE | FOREGROUND_INTENSITY);
	}
	else
	{
