...
namespace driver {
	...
	// my cool ones below -breww

	uint64_t free_memory(SOCKET connection, uint32_t process_id, uint64_t base);
	uint64_t protect_memory(SOCKET connection, uint32_t process_id, uint64_t base, uint64_t size, ULONG protection);
	uint64_t allocate_memory(SOCKET connection, uint32_t process_id, uint64_t size, ULONG protect);
