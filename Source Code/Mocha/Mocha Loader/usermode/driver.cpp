...
static uint32_t copy_memory(
	const SOCKET	connection,
	const uint32_t	src_process_id, // target process id
	const uintptr_t src_address, // target address
	const uint32_t	dest_process_id, // our process id
	const uintptr_t	dest_address, // buffer
	const size_t	size)
{
	Packet packet{ };
...
uint64_t driver::free_memory(const SOCKET connection, const uint32_t process_id, const uint64_t base)
{
	Packet packet{ };

	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_free_process_memory;
...
uint64_t driver::protect_memory(
	const SOCKET connection, 
	const uint32_t process_id, 
	const uint64_t base, 
	const uint64_t size, 
...
)
{
	...
	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_protect_process_memory;
...
uint64_t driver::allocate_memory(
	const SOCKET connection, 
	const uint32_t process_id, 
	const uint64_t size, 
	const ULONG protect
)
{
	...
	packet.header.magic = packet_magic;
	packet.header.type = PacketType::packet_allocate_process_memory;
