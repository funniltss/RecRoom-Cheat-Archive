...
bool ImAdd::RadioFrameIconColor(const char* label, ImTextureID user_texture_id, int* v, int current_id, const ImVec4& custom_icon_color, const ImVec2& size_arg, const ImVec2& adjust_icon_pos)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec4 colIcon = custom_icon_color;
    if (hovered || held || active)
        colIcon = ImVec4(custom_icon_color.x * 1.5f, custom_icon_color.y * 1.5f, custom_icon_color.z * 1.5f, custom_icon_color.w);

    ImVec4 colLabel = GetStyleColorVec4((hovered || held || active) ? ImGuiCol_SliderGrab : ImGuiCol_TextDisabled);
...
bool ImAdd::RadioFrame(const char* label, int* v, int current_id, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
std::vector<ImVec2> highlightStack; // yes i could use ImGui stack. No fuck you i will not.

bool ImAdd::BeginHighlightGroup() {
    ImGui::BeginGroup();
    highlightStack.push_back(ImGui::GetCursorScreenPos());
    return true;
}

bool ImAdd::EndHighlightGroup(float lineThickness, float offset, ImVec4 color) {
    ImGui::EndGroup();
    if (!highlightStack.empty()) {
        ImVec2 highlightStart = highlightStack.back();
        highlightStack.pop_back();
        ImVec2 highlightEnd = ImGui::GetCursorScreenPos();

        ImDrawList* drawList = ImGui::GetWindowDrawList();
        drawList->AddLine(ImVec2(highlightStart.x - offset, highlightStart.y), ImVec2(highlightEnd.x - offset, highlightEnd.y), ImGui::ColorConvertFloat4ToU32(color), 2.0f);
    }
    return true;
}

bool ImAdd::ButtonAutoWidth(const char* label, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float padding = style.FramePadding.x * 2.0f; // Horizontal padding
    ImVec2 size(label_size.x + padding, label_size.y + style.FramePadding.y * 2.0f); // Auto width based on text
