...
namespace SilkRoad::PatternScan 
{
    ...
    uintptr_t GetSteamModule() {
        return (uintptr_t)GetModuleHandleA(_("GameOverlayRenderer64.dll"));
    }

    uintptr_t GetGameAssemblyModule() {
        return (uintptr_t)GetModuleHandleA(_("GameAssembly.dll"));
    }

    uintptr_t GetRecRoomDLLModule() {
        return (uintptr_t)GetModuleHandleA(_("RecRoom.exe.dll"));
    }
    
    void CreateHook(__int64 iAddr, __int64 iFunction, __int64* iOriginal) {
        static uintptr_t pHookAddr;
        if (!pHookAddr) pHookAddr = FindPattern(GetSteamModule(), (_("48 ? ? ? ? 57 48 83 EC 30 33 C0")));
