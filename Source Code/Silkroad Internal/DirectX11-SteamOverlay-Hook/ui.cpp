...
void c_custom::tab_area(const char* str_id, ImVec2 size, std::function < void() > content) {
    ...
    BeginChild(std::string(str_id).append(_(".child")).c_str(), ImVec2(value->second.width, size.y));
    ...
    if (expand_button(_("Expand"), value->second.should_expand, GetStyle().WindowRounding, ImDrawFlags_RoundCornersTopLeft))
...
void c_custom::begin_child(const char* name, ImVec2 size) {
    ...
    BeginChild(std::string(name).append(_(".main")).c_str(), size, false, ImGuiWindowFlags_NoScrollbar);
